...
class Compile: # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def set_config(self, config):
        ...
        if 'PEDAGOGY_STATES' in self.options:
            self.pedagogy_states = self.options['PEDAGOGY_STATES']
    ...
    def pedagogy_question_state(self, index):
        ...
        if self.question_yet_solved(index):
            pedagogy_append(html_class, 'good')
        elif (index <= self.current_question_max
                or not self.options['sequential']
                or self.options['GRADING']
                or self.options['ANSWERS'][index]):
            pedagogy_append(html_class, 'possible')
        else:
            pedagogy_append(html_class, 'locked')
            link = ''
    ...
    def pedagogy_node_content(self, node, depth):
        ...
        if index is not None:
            ...
            pedagogy_append(html_class, state)
        node_state = self.pedagogy_node_state(node)
        pedagogy_append(html_class, 'progress_' + node_state)
        title = node['title'] or node['id']
        # Keep metadata generated by the first recursive-pedagogy revision usable.
        points = node['points']
        if 'display_points' in node:
            points = node['display_points']
        elif node['children']:
        ...
        for child in node['children']:
            pedagogy_append(content, self.pedagogy_node_content(child, depth + 1))
        return ''.join(content)

    def pedagogy_node_state(self, node):
        """Return own state for questions and aggregate state for containers."""
        child_states = []
        if node['answer_index'] is not None:
            pedagogy_append(child_states,
                             self.pedagogy_states[node['id']] or 'unvisited')
        for child in node['children']:
            pedagogy_append(child_states, self.pedagogy_node_state(child))
        if not child_states:
            return 'unvisited'
        all_completed = True
        all_saved = True
        ...
        for state in child_states:
            if state != 'completed':
                all_completed = False
            if state not in ('saved', 'completed'):
                all_saved = False
            if state in ('started', 'saved', 'completed'):
                has_started = True
            if state == 'visited':
                has_visited = True
        if all_completed:
