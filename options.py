...
┗━━━━━━━━┹────────────────────┴────────────────────┘</pre>"""],
    ...
    ['display_executor_copy',0, "Display a button to copy the execution result"],
    ['run_tester',           1, "Run the question tester after execution"],
    ['display_expected_answer',1, "Send the expected answer to the interface"],
    ['sql_result_format', 'html', "SQL result format: html or text"],
    ['sql_single_select', 0, "Require one SELECT statement ending with a semicolon"],
    ...
    ['executor_copy_button', 'Copier', "Execution result copy button label"],
    ...
    ['compiler', '',        "Compiler to use (should not be changed):<br>«g++» «gcc» «racket» «coqc» «prolog» «cargo»"],
    ['compile_options', [], "C Compile options:<br>«-Wall» «-pedantic» «-pthread» «-std=c++11» «-std=c++20»"],
    ['ld_options', [],      "C Libraries to link with:<br>«-lm»"],
    ['language', '',        "Language to use for syntaxic coloring:<br> «cpp» «python» «coq» «rust»..."],
    ['extension', '',       "Source code filename extension for ZIP and GIT: «cpp», «py», «v» «rs»..."],
